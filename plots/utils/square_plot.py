...
def add_job_square(ax, id, job, pattern, color, resources):
    start_time = job['scheduled'][-1]
    end_time = job['completion']
    label = False
    for nvme in job["volumes"]:
        nvmeid = resources[int(job['rackid'][-1])][int(nvme)]
        plot_square(ax, id, int(start_time), int(end_time), [nvmeid], pattern, color, label)
        label = False
    return ax
